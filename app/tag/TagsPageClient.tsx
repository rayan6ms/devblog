...
export default function TagsPageClient({
	...
	const mainTagOptions = useMemo(
		() => mainTags.filter((tag) => matchesTagSearch(tag, normalizedTagQuery)),
		[mainTags, normalizedTagQuery],
	);
	const otherTagOptions = useMemo(
		() => otherTags.filter((tag) => matchesTagSearch(tag, normalizedTagQuery)),
		[normalizedTagQuery, otherTags],
	);
	...
	const tagLabelMap = useMemo(
		() =>
			new Map([...mainTags, ...otherTags].map((tag) => [tag.slug, tag.name])),
		[mainTags, otherTags],
	);
	...
	const marqueeMainTags =
		mainTags.length > 0
			? mainTags.slice(0, 10).map((tag) => tag.name)
			: mainTags.map((tag) => tag.name);
	const marqueeOtherTags =
		otherTags.length > 0
			? otherTags.slice(0, 14).map((tag) => tag.name)
