					...
					{tags.map((tag) => {
						...
						return (
							<button
								key={tag.slug}
								type="button"
								className={`capitalize flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left transition-colors ${
									isSelected
							...
							{featuredTags.map((tag) => {
								...
								return (
									<button
										key={tag.slug}
										type="button"
										className={`capitalize rounded-full border px-3 py-2 text-sm transition-colors ${
											isSelected
												? "border-purpleContrast/50 bg-purpleContrast/15 text-wheat"
