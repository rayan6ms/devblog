...
export default function MovingMountains() {
	...
	useEffect(() => {
		hueModeRef.current = hueMode;
	}, [hueMode]);
	useEffect(() => {
		singleHueRef.current = hexToHsl(singleHex).h;
	}, [singleHex]);
	useEffect(() => {
	...
	useEffect(() => {
		...
		setSketch(() => (p) => {
			...
			const paletteFrom = (base: number, kind: Palette) => {
				if (kind === "analogous")
					return [base, (base + 20) % 360, (base + 340) % 360];
			...
			const hueFor = (x: number, y: number, z: number, t: number) => {
				...
				if (mode === "random")
					h = (x * 37.7 + y * 19.3 + Math.floor(t * 10) * 11.1) % 360;
				if (mode === "randomLerp") {
				...
				if (nMode === "hue")
					h = (h + p.noise(x * ns, y * ns, t * 0.2) * 80) % 360;
				return h;
			};
			...
			const zJitter = (x: number, y: number, t: number) => {
				...
				const ns = Math.max(0.0001, noiseScaleRef.current);
				if (nMode === "liquid")
					return (p.noise(x * ns, y * ns + t * 0.3) - 0.5) * 45;
				if (nMode === "granular")
					return (p.noise(x * ns * 2.2, y * ns * 2.2, t * 0.6) - 0.5) * 18;
				return 0;
			};
			...
			p.windowResized = () => {
				...
				p.perspective(
					p.PI / 3,
					Math.max(1, p.width) / Math.max(1, p.height),
					0.1,
					800,
				);
			};
			...
			p.draw = () => {
				...
				for (let y = 0; y < rows - 1; y++) {
					makeRow(
						startY + (y + 1) * (noiseScaleRef.current * 1.6667 || 0.1),
						terrainB,
					);
