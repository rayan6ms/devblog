	...
	useEffect(() => {
		...
		setSketch(() => (p: any) => {
			...
			const spawnParticles = (
				origin: Vec,
				dir: Vec,
				kind: "muzzle" | "fuse" | "impact",
			) => {
				const n = kind === "impact" ? 24 : kind === "muzzle" ? 18 : 8;
				for (let i = 0; i < n; i++) {
					const jitter =
						(Math.random() - 0.5) * (kind === "impact" ? 1.8 : 1.2);
					const ang = Math.atan2(dir.y, dir.x) + jitter;
					const spd =
						(kind === "impact" ? 2.2 : kind === "muzzle" ? 1.6 : 1.0) *
						(0.6 + Math.random() * 0.8);
					...
					const color =
						kind === "impact"
							...
							: i % 2
								...
								: "hsl(55 100% 80%)";
					const size =
						kind === "impact" ? 2 + Math.random() * 3 : 2 + Math.random() * 2;
					particles.push({
			...
			const integrate = (b: Ball) => {
				...
				for (let i = 0; i < SUBSTEPS; i++) {
					...
					if (distToCenter <= PLANET_RADIUS + BALL_RADIUS * 0.8) {
						...
						const surfPos = add(
							planetCenter,
							mul(surfDir, PLANET_RADIUS + BALL_RADIUS * 0.1),
						);
			...
			const computePreview = () => {
				...
				let pos = muzzlePos();
				let vel = v(
					Math.cos(aimAngleGlobal) * SHOT_SPEED,
					Math.sin(aimAngleGlobal) * SHOT_SPEED,
				);
			...
			p.draw = () => {
				...
				const cdFrac = Math.min(
					1,
					Math.max(0, (performance.now() - lastShot) / COOLDOWN_MS),
				);
				if (cdFrac < 1) {
					const ropeLen = 50;
					const segs = 18;
					const tBurn = cdFrac;
					const burnedUntil = Math.floor(segs * tBurn);
					const pts: Vec[] = [];
					...
					p.noFill();
					p.beginShape();
					for (let i = burnedUntil; i <= segs; i++)
					...
					spawnParticles(
						v(tip.x, tip.y),
						v(Math.cos(aimAngleGlobal), Math.sin(aimAngleGlobal)),
						"fuse",
					);
				}
				p.pop();
