		...
		setSketch(() => (p) => {
			...
			class Player {
				...
				addOrUpdateAbility(abilityName: AbilityName) {
					...
					if (ability) ability.upgrade();
						...
						this.abilities.push(
							new Ability(abilityName, abilitiesInfo[abilityName].effect),
						);
				}
				shoot() {
						...
						for (let q = 0; q < shotsPerVolley; q++) {
							...
							setTimeout(() => {
								...
								for (let m = 0; m < multiShotExtraBullets + 1; m++) {
									const angleOffset =
										((m - Math.floor(shotsPerVolley / 2)) * p.PI) /
										angleIncrement;
									const angle =
										p.atan2(target.y - this.y, target.x - this.x) + angleOffset;
									const pierceLevel = this.getAbilityLevel("pierce");
									bullets.push(
										new Bullet(this.x, this.y, angle, this.damage, pierceLevel),
									);
								}
							}, q * delayBetweenShots);
						}
				...
				display() {
					...
					if (target) {
						...
						p.rect(10, -3, 20, 6);
						const gripPosition =
							angle > p.HALF_PI || angle < -p.HALF_PI ? -8 : 4;
			...
			class Enemy {
				...
				adjustColorForLevel(level: number) {
					if (level >= 7)
						this.color = p.color(255 - Math.min(200, (level - 6) * 10), 0, 0);
			...
			class Bullet {
					...
					for (let i = enemies.length - 1; i >= 0; i--) {
						...
						if (distFromEnemy < this.size + enemies[i].size / 2) {
							...
							if (enemies[i].lives <= 0) {
								...
								xpItems.push(
									new XPItem(enemies[i].x, enemies[i].y, xpShape, xpValue),
								);
								enemies.splice(i, 1);
								enemyKillCount++;
							...
							if (this.piercedEnemies.length >= this.pierceLevel + 1)
								return true;
							collision = true;
						}
					}
					return (
						collision && this.piercedEnemies.length >= this.pierceLevel + 1
					);
				}
				display() {
					p.fill(100);
				...
				isOutOfScreen() {
					return (
						this.x < 0 || this.x > p.width || this.y < 0 || this.y > p.height
					);
				}
			}
			...
			class XPItem {
				...
				getColor() {
					switch (this.shape) {
						case "pentagon":
							return p.color(144, 238, 144);
						case "hexagon":
							return p.color(233, 238, 144);
						case "octagon":
							return p.color(189, 144, 238);
						case "decagon":
							return p.color(144, 235, 238);
						default:
							return p.color(255);
					}
				}
			...
			function assignPlayerAbility() {
				...
				if (currentLevel === 1 || [7, 14, 21].includes(currentLevel)) {
					const newAbilities = availableAbilities.filter(
						(ab) => !player.abilities.some((pab) => pab.name === ab.name),
					);
			...
			function createEnemy() {
				const extraLife = Math.min(
					7,
					Math.floor(Math.max(0, currentLevel - 6) / 7) * 2,
				);
			...
			function drawLevelAndXPBar() {
				...
				p.rect(
					p.width * 0.12,
					35,
					barWidth * p.constrain(xpProgress, 0, 1),
					barHeight,
					5,
				);
			}
			...
			function drawStats() {
				...
				player.abilities.forEach((ability, index) => {
					const cap = ability.name.replace(/\b([a-z])/g, (m) =>
						m.toUpperCase(),
					);
			...
			function checkPlayerDamage() {
				for (let i = enemies.length - 1; i >= 0; i--) {
					if (
						p.dist(player.x, player.y, enemies[i].x, enemies[i].y) <=
			...
			function updateLevel() {
				if (currentXP >= xpNeededForNextLevel) {
					...
					player.weaponCooldown = Math.max(
						200,
						1000 - Math.min(720, 30 * currentLevel),
					);
					if ([7, 14, 21].includes(currentLevel)) {
						player.lives += 1;
						player.damage += 1;
			...
			function updateGameLogic() {
				...
				if (enemies.length < 5)
					enemySpawnRate = Math.max(
						500,
						baseEnemySpawnRate - enemies.length * 100,
					);
				...
				for (let i = bullets.length - 1; i >= 0; i--) {
					bullets[i].move();
					if (bullets[i].checkCollision() || bullets[i].isOutOfScreen())
						bullets.splice(i, 1);
				}
			...
			function drawGame() {
				if (frenesi) {
					...
					if (p.millis() - frenesiTimer > 10000) {
						frenesi = false;
						enemies.forEach((enemy) => {
							enemy.lives = Math.max(
								3,
								enemy.originalLives + extraLifeBasedOnLevel(currentLevel),
							);
