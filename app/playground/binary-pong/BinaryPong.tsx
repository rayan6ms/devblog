...
export default function BinaryPong() {
	...
	const sketch = useCallback<Sketch>((p) => {
		...
		function placeBalls() {
			...
			const rightX = randBetween(
				canvasSizeLocal * 0.68,
				canvasSizeLocal * 0.82,
			);
		...
		function collideAndFlipSingle(ball: Ball, dx: number, dy: number) {
			...
			candidates.sort((a, b) => {
				...
				const da = (ax - ball.x) * (dx || 1e-6) + (ay - ball.y) * (dy || 1e-6);
				const db = (bx - ball.x) * (dx || 1e-6) + (by - ball.y) * (dy || 1e-6);
				return da - db;
			});
			...
			for (const [c, r] of candidates) {
				...
				const nearestX = Math.max(
					c * cellSize,
					Math.min(nx, (c + 1) * cellSize),
				);
				const nearestY = Math.max(
					r * cellSize,
					Math.min(ny, (r + 1) * cellSize),
				);
		...
		(p as any).__setBaseSpeed = (v: number) => {
			baseSpeedPxPerFrame = v;
		};
		(p as any).__setPaused = (v: boolean) => {
			isPaused = v;
		};
		(p as any).__setSpeedMult = (v: number) => {
			speedMult = v;
		};
	...
	return (
		<div className="w-full h-full flex flex-col">
			...
			<div className="w-full pb-2">
				...
				>
					<div className="relative w-1/2 max-w-[420px] h-10 rounded-lg overflow-hidden border border-zinc-800/70">
						<div
							className="absolute inset-y-0 left-0 w-1/2 flex items-center justify-center font-semibold"
							style={{ color: DARK_CYAN, background: ICE_OFF_WHITE }}
						>
							Ice: {scores.ice}
						</div>
						<div
							className="absolute inset-y-0 right-0 w-1/2 flex items-center justify-center font-semibold"
			...
			<div
				ref={hostRef}
				className="relative flex-1 grid place-items-center p-2"
			>
					...
					<div className="pointer-events-none absolute inset-0 flex items-center justify-center">
						<div className="pointer-events-auto bg-zinc-900/70 backdrop-blur rounded-2xl px-6 py-5 border border-zinc-700/60 text-center shadow-2xl">
							...
							<div className="text-zinc-400 text-sm mb-4">
								the board has been fully conquered
							</div>
