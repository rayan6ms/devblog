...
export default function LabyrinthExplorer() {
	...
	useEffect(() => {
		...
		setSketch(() => (p) => {
			...
			class Cell {
				...
				show() {
					const x =
						this.i * w * state.mapScale + (p.width - cols * w * state.mapScale);
			...
			function generateMaze() {
				...
				w = Math.max(
					40,
					Math.min(100, Math.floor(mapPixels / state.mapScale / 12)),
				);
			...
			function buildWallSegments() {
				...
				for (let j = 0; j < rows; j++) {
					for (let i = 0; i < cols; i++) {
						...
						const y1 = (j + 1) * w;
						if (c.walls[0])
							wallSegments.push({ x1: x0, y1: y0, x2: x1, y2: y0 });
						if (c.walls[1])
							wallSegments.push({ x1: x1, y1: y0, x2: x1, y2: y1 });
						if (c.walls[2])
							wallSegments.push({ x1: x1, y1: y1, x2: x0, y2: y1 });
						if (c.walls[3])
							wallSegments.push({ x1: x0, y1: y1, x2: x0, y2: y0 });
					}
				}
			}
			...
			function canMove(oldX: number, oldY: number, newX: number, newY: number) {
				...
				if (newI === oldI + 1 && newJ === oldJ)
					return !oldCell.walls[1] && !newCell.walls[3];
				if (newI === oldI - 1 && newJ === oldJ)
					return !oldCell.walls[3] && !newCell.walls[1];
				if (newJ === oldJ + 1 && newI === oldI)
					return !oldCell.walls[2] && !newCell.walls[0];
				if (newJ === oldJ - 1 && newI === oldI)
					return !oldCell.walls[0] && !newCell.walls[2];
				return false;
			}
			...
			function render3D() {
				...
				for (let i = 0; i < p.width; i++) {
					...
					for (const seg of wallSegments) {
						const r = raySegmentIntersection(
							player.x,
							player.y,
							dx,
							dy,
			...
			p.draw = () => {
				if (!roundActive) {
					...
					if (round > maxRounds) {
						p.text(
							"You win! Rounds completed: " + maxRounds,
							p.width / 2,
							p.height / 2,
						);
					} else {
	...
	return (
		<div ref={hostRef} className="w-full h-full relative bg-black">
