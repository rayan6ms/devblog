	...
	useEffect(() => {
		...
		setSketch(() => (s) => {
			...
			const drawBird = () => {
				...
				const rot = s.constrain(
					s.map(birdV, -8, MAX_VELOCITY, -0.35, 0.6),
					-0.6,
					0.6,
				);
				s.rotate(rot);
			...
			const updateAutoplay = () => {
				...
				const next = pipes.find((p) => p.x > BIRD_X - 10);
			...
			<div className="pointer-events-auto absolute right-2 top-2 flex gap-2">
				<button
					className={`rounded-xl px-3 py-2 text-sm font-medium shadow ${
						autoplay ? "bg-green-500 text-black" : "bg-white/90"
					}`}
				...
				<button
					className={`rounded-xl px-3 py-2 text-sm font-medium shadow ${
						paused ? "bg-yellow-400 text-black" : "bg-white/90"
					}`}
