...
export default function Terminal() {
	...
	const updateSuggestion = (value) => {
		...
		const suggestedCommand = commandNames.find((command) =>
	...
	const processCommand = (commandString) => {
		...
		for (let i = 0; i < commandSegments.length; i++) {
			...
			if (i < commandSegments.length - 1 && commandString.includes(">")) {
				...
				const fileDir = getPathContent(
					resolvePath(currentDirectoryPath, filePath),
				);
				if (fileDir) {
			...
			<div className="history">
				{history.map((item) => (
					<div key={item.command}>
						{item.command && (
							<div className="command">rayan@terminal:~$ {item.command}</div>
						)}
