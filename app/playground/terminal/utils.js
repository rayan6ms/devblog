...
export const resolvePath = (currentPath, targetPath) => {
	...
	const resolvedPathArr = targetPath.startsWith("/") ? [] : currentPathArr;
...
function tokenize(expr) {
	return expr.match(
		/([0-9.]+|[-+*/^%(){}[\]**]|exp|log\d*|sin|cos|tan|abs|round|floor|ceil)/g,
	);
}
		...
		if (tokens[index] && tokens[index].startsWith("log")) {
			const base =
				tokens[index].length > 3
					? parseInt(tokens[index].slice(3), 10)
					: Math.E; // Default to base E if no base provided
			index++;
