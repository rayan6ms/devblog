...
function materialTotals(g: Chess) {
	...
	for (let f = 0; f < 8; f++)
		for (let r = 0; r < 8; r++) {
			...
			const pc = g.get(sq);
			if (!pc) continue;
			if (pc.color === "w") w += PIECE_VALUE[pc.type];
			else b += PIECE_VALUE[pc.type];
		}
	return { w, b };
}
...
function materialScore(g: Chess) {
	...
	for (let f = 0; f < 8; f++)
		for (let r = 0; r < 8; r++) {
			...
			if (!pc) continue;
			pc.color === "w"
				? (w += PIECE_VALUE[pc.type])
				: (b += PIECE_VALUE[pc.type]);
		}
	return w - b;
}
...
function useStockfish() {
	...
	const go = (
		...
		w.postMessage(
			opts.movetime ? `go movetime ${opts.movetime}` : `go depth ${depth}`,
		);
...
export default function ChessP5() {
			...
			if (document.hidden) {
				...
				if (hiddenStartedAtRef.current != null) {
					turnStartRef.current +=
						performance.now() - hiddenStartedAtRef.current;
		...
		setSketch(() => (p) => {
			...
			p.draw = () => {
				...
				if (rr.overlay || rr.paused) {
					...
					p.fill("#fff");
					p.textSize(Math.floor(p.width * 0.06));
				...
				if (x >= 0 && y >= 0 && x < p.width && y < p.height) {
					const fDisp = Math.floor(x / q),
						rDisp = Math.floor(y / q);
	...
	function rebuildFromLog() {
		...
		for (const m of moveLogRef.current) {
			if (m.captured) {
				(m.color === "w" ? tk.b : tk.w).push(m.captured as PieceSymbol);
			}
			const md = {
				san: m.san || "",
				piece: (m.piece as PieceSymbol) || "p",
				timeMs: m.timeMs || 0,
				acc: m.acc ?? 50,
			};
			if (m.color === "w") {
	...
	function applyMove(m: {
		...
		const ok = g
			.moves({ verbose: true })
			.some(
				(mm) => (mm.from as Square) === m.from && (mm.to as Square) === m.to,
			);
		if (!ok) return;

		const piece = g.get(m.from);
		const needsPromo =
			piece?.type === "p" && (m.to[1] === "1" || m.to[1] === "8");

		const mv = g.move({
			from: m.from,
			to: m.to,
			promotion: needsPromo ? "q" : m.promotion,
		});
		if (!mv) return;
		...
		if ((mv as any).captured) {
			...
			if ((mv as any).color === "w")
				setTaken((prev) => ({ ...prev, b: [...prev.b, cap] }));
	...
	function undo() {
		...
		const g = log.length
			? new Chess(log[log.length - 1].fenAfter)
			: new Chess();
		setGame(g);
	...
	function redo() {
		...
		setMoves([]);
		moveSetRef.current = new Set();
		const out = gameOutcome(new Chess(next.fenAfter));
		if (out) setOverlay(out);
		rebuildFromLog();
	}
	...
	function surrender() {
		const winner =
			rRef.current.game.turn() === "w" ? "Black wins" : "White wins";
	...
	function cycleHints() {
		...
		setHintsMode((prev) => {
			const next: HintsMode =
				prev === "off" ? "once" : prev === "once" ? "auto" : "off";

			const rr = rRef.current;
			if (
				rr.playAs !== null &&
				rr.game.turn() === rr.playAs &&
				(next === "once" || next === "auto")
			) {
				requestHint();
			}
					...
					<div className="flex-1 flex flex-col justify-start gap-3 min-h-0">
						<div className="flex items-start justify-between gap-2 h-20 shrink-0">
							<div className="flex flex-wrap gap-1 text-2xl leading-none overflow-hidden">
								{(effectivePlayAs === "w" ? taken.w : taken.b).map((t, i) => (
									<span key={i}>
										{GLYPH[t][effectivePlayAs === "w" ? "b" : "w"]}
									</span>
								))}
							</div>
						...
						<div
							ref={moveListRef}
							className="flex-1 overflow-auto -mx-3 min-h-64"
						>
							{pairs.map((row, i) => {
								const even = i % 2 === 1;
								...
								return (
									<div
										key={row.index}
										className={`${rowCls} py-1 pr-2 flex items-center gap-2`}
									>
						...
						<div className="flex items-end justify-between gap-2 h-20 shrink-0">
							<div className="flex flex-wrap gap-1 text-2xl leading-none overflow-hidden">
								{(effectivePlayAs === "w" ? taken.b : taken.w).map((t, i) => (
									<span key={i}>
										{GLYPH[t][effectivePlayAs === "w" ? "w" : "b"]}
									</span>
								))}
							</div>
							<div className="select-none">
...
function drawBoard(
	...
	if (last) {
		[last.from, last.to].forEach((sq) =>
			tintSquare(p, sq, flipped, q, COLORS.lastMove),
		);
	}
...
function drawArrow(
	...
	const fx1 = flipped ? 7 - f1 : f1,
		ry1 = flipped ? r1 : 7 - r1;
	const fx2 = flipped ? 7 - f2 : f2,
		ry2 = flipped ? r2 : 7 - r2;
	...
	const sLx = sx - px * halfS,
		sLy = sy - py * halfS;
	const sRx = sx + px * halfS,
		sRy = sy + py * halfS;
	const bLx = bx - px * halfH,
		bLy = by - py * halfH;
	const bRx = bx + px * halfH,
		bRy = by + py * halfH;
