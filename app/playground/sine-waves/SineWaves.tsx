...
export default function SineWaves() {
	...
	const sketch = useCallback<Sketch>((p) => {
		...
		const resetAngles = () => {
			angles = [];
			for (let i = 0; i < settingsRef.current.numCircles; i++)
				angles.push(-p.HALF_PI);
		};
		...
		p.draw = () => {
			...
			if (s.showGuides) {
				...
				if (s.waveAxis === "y")
					p.line(xCurrent, yCurrent, p.width / 2, yCurrent);
			...
			if (s.showGuides) {
				p.circle(
					s.waveAxis === "y" ? p.width / 2 : xCurrent,
					s.waveAxis === "y" ? yCurrent : p.height / 2,
					8,
				);
			}
			...
			if (s.waveAxis === "y") {
				for (let i = 0; i < wave.length; i++)
					p.vertex(p.width / 2 + i, wave[i]);
			} else {
				for (let i = 0; i < wave.length; i++)
					p.vertex(wave[i], p.height / 2 + i);
			}
			p.endShape();

			const maxTrail = Math.floor(
				(s.waveAxis === "y" ? p.width : p.height) * s.trailRatio,
			);
