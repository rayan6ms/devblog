...
const installP5GlobalBridge = (p: any) => {
	...
	defineProp("height", () =>
		getVM()?.active ? getVM().baseH : getP()?.height,
	);
	...
	if (!("keyIsDown" in g)) {
		Object.defineProperty(g, "keyIsDown", {
			value: (...a: any[]) => getP()?.keyIsDown?.(...a),
			configurable: true,
		});
	}
	...
	useEffect(() => {
		...
		(async () => {
			...
			const sketch = (p: any) => {
				...
				const VIRTUAL = { w: 1, h: 1 };

				(globalThis as any).__p5vm = {
					active: true,
					baseW: VIRTUAL.w,
					baseH: VIRTUAL.h,
					s: 1,
				...
				const allowFSKey = (k: string) =>
					k === "Escape" || k === "Esc" || k === "f" || k === "F";
				...
				document.addEventListener("fullscreenchange", () => {
					...
					(globalThis as any).__p5hostEl =
						fsEl === p.canvas ? (p.canvas as HTMLElement) : hostEl();
					...
					updateVirtualFromHost();
					try {
						(p.canvas as HTMLCanvasElement).focus();
					} catch {}
				});
				...
				function setup() {
					...
					(p.canvas as HTMLCanvasElement).addEventListener(
						"keydown",
						onKeyDown,
					);
					...
					p.remove = () => {
						...
						try {
							(p.canvas as HTMLCanvasElement).removeEventListener(
								"keydown",
								onKeyDown,
							);
							(p.canvas as HTMLCanvasElement).removeEventListener(
