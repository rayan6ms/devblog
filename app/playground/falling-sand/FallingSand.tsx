		...
		setSketch(() => (pg) => {
			...
			const applyPalette = (base: number) => {
				...
				if (cfg.palette === "analogous") {
					...
					return wrap360(
						b + offsets[Math.floor(Math.random() * offsets.length)] + jitter(),
					);
				}
			...
			const svFor = (pg: p5Type, c: number, r: number) => {
				...
				if (cfg.palette !== "mono") {
					const s255 =
						cfg.hueMode === "single" ? Math.round(cfg.singleS * 255) : 255;
					const v255 =
						cfg.hueMode === "single" ? Math.round(cfg.singleV * 255) : 255;
					return { s255, v255 };
				}
					...
					if (cfg.solid) {
						...
						if (grid[id] === -1) {
							...
							staticLayer.fill(
								hue,
								s255,
								Math.max(12, Math.round(v255 * vMul)),
							);
			...
			const bakeStatic = (pg: p5Type, p: Particle) => {
				...
				staticLayer.fill(
					p.hue,
					s255,
					Math.max(0, Math.min(255, Math.round(v255 * p.vMul))),
				);
				staticLayer.square(
					p.col * settingsRef.current.cell,
					p.row * settingsRef.current.cell,
					settingsRef.current.cell,
				);
			};
			...
			const sketch = (pg: p5Type) => {
				...
				const allocate = (width: number, height: number) => {
					...
					const cw = Math.max(
						cfg.cell,
						Math.floor(width / cfg.cell) * cfg.cell,
					);
					const ch = Math.max(
						cfg.cell,
						Math.floor(height / cfg.cell) * cfg.cell,
					);
					...
					if (cfg.brush === "spray") {
						...
						for (let dy = -brushExtent; dy <= brushExtent; dy++) {
							for (let dx = -brushExtent; dx <= brushExtent; dx++) {
								...
								if (
									c >= 0 &&
									c < Math.floor(pg.width / cfg.cell) &&
									r >= 0 &&
									r < Math.floor(pg.height / cfg.cell)
								) {
				...
				const update = () => {
					...
					const driftMul =
						nm === "liquid" ? 2.4 : nm === "granular" ? 0.55 : 1.0;
					const sleepThresh =
						nm === "liquid"
					...
					for (let t = 0; t < len; t++) {
						...
						const driftNoise =
							nm === "hue" || nm === "liquid" ? fastNoise(p.col, p.row, z1) : 0;

						const drift =
							Math.sign(driftNoise) *
							(Math.abs(driftNoise) > 0.02 ? 1 : 0) *
							driftMul;
						const granJitter =
							nm === "granular"
								? Math.random() < 0.25
						...
						for (let s = 0; s < steps; s++) {
							if (tryMoveTo(p5Ref.current!, i, p.col, p.row + 1)) {
								moved = true;
								continue;
							}

							const tryDiag = (dc: number) =>
								tryMoveTo(p5Ref.current!, i, p.col + dc, p.row + 1);
							if (leftFirst) {
								if (tryDiag(-1)) {
									moved = true;
									continue;
							...
							if (
								drift !== 0 &&
								tryMoveTo(p5Ref.current!, i, p.col + drift, p.row)
							) {
								moved = true;
								continue;
							}
							if (
								granJitter !== 0 &&
								tryMoveTo(p5Ref.current!, i, p.col + granJitter, p.row)
							) {
								moved = true;
								continue;
							}
						...
						if (!moved && nm === "liquid") {
							...
							if (tryMoveTo(p5Ref.current!, i, p.col + dir, p.row))
								moved = true;
						}
				...
				const drawActive = () => {
					...
					for (let k = 0; k < active.length; k++) {
						...
						pg.fill(
							p.hue,
							sv.s255,
							Math.max(0, Math.min(255, Math.round(sv.v255 * p.vMul))),
						);
						const cfg = settingsRef.current;
			...
			<div className="flex flex-wrap items-center gap-3">
				...
				<div className="flex items-center gap-2">
					...
					<div className="flex items-center gap-2">
						...
						/>
						<span className="text-xs text-zinc-400 w-fit text-right">
							{brushDiam}
						</span>
					</div>
					<div className="flex items-center gap-2">
