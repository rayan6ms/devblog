...
function SnakeGame() {
  ...
  function getAStarDirections(head) {
    ...
    if (path.length === 0 || path.length === 1) {
      return [getHamiltonianDirection(head)];
    }
    ...
    if (dx === 1) possibleDirections.push(directions.RIGHT);
    if (dx === -1) possibleDirections.push(directions.LEFT);
    if (dy === 1) possibleDirections.push(directions.DOWN);
    if (dy === -1) possibleDirections.push(directions.UP);
  ...
  function updateGame() {
    ...
    if (segments.some(seg => seg.x === apple.x && seg.y === apple.y)) placeApple();
    if (autoPlay) {
      const possibleDirections = getAStarDirections(segments[0]);
      let chosenDirection = possibleDirections[0];

      if (chosenDirection === getHamiltonianDirection(segments[0])) {
        direction = chosenDirection;
      } else {
        direction = getHamiltonianDirection(segments[0]);
      }
    }
  ...
  function changeDirection(e) {
    ...
    if (e.key === ' ' || e.key === 'Escape') setPaused(!paused);

    if (!paused) setAutoPlay(false);
