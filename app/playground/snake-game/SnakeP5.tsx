...
function buildDirectionMatrix(n: number): Dir[][][] {
	const M: Dir[][][] = Array.from({ length: n }, () =>
...
export default function SnakeP5({
	boardSize = 20,
	cellPx = 40,
	initialSpeed = 5,
	className = "",
	...
	useEffect(() => {
		...
		setSketch(() => (p) => {
			...
			function buildCellTreeParentsFromSnake() {
				const CW = N >> 1,
					CH = N >> 1;
				const parents: (Point | null)[][] = Array.from({ length: CH }, () =>
					Array(CW).fill(NOT_VIS),
				);
			...
			function normDir(a: Point, b: Point): Dir {
			...
			function aStarWithPenalties(
				...
				const h = (p: Point) =>
					(Math.abs(p.x - goal.x) + Math.abs(p.y - goal.y)) * STEP;
			...
			function floodReachable(
				...
				while (head < tail) {
					const u = q[head++],
						a = toPoint(u);
					pushIfOK(a, DIRS.RIGHT);
					pushIfOK(a, DIRS.LEFT);
					pushIfOK(a, DIRS.DOWN);
					pushIfOK(a, DIRS.UP);
				}
				return seen;
			}
			...
			function autoDir(): Dir | null {
				...
				const { path, dist, came } = aStarWithPenalties(
					head,
					apple,
					parents,
					blocked,
				...
				if (!path || path.length === 0) {
					...
					for (let i = 0; i < nb.length; i++)
						if (!occ.has(`${nb[i].x},${nb[i].y}`)) return normDir(head, nb[i]);
					return null;
				}
				...
				if (stepsR.current > 110) {
					...
					const parentsAfter = (() => {
						const CW = N >> 1,
							CH = N >> 1;
						const p2: (Point | null)[][] = Array.from({ length: CH }, () =>
							Array(CW).fill(NOT_VIS),
						);
						let parent: Point = { x: -2, y: -2 };
				...
				if (
					insideStreak >= 3 &&
					!(appleCell.x === curCell.x && appleCell.y === curCell.y)
				) {
			...
			p.draw = () => {
				...
				if (mps !== speedRef.current) {
					mps = speedRef.current;
					stepMs = 1000 / mps;
				}
	...
	return (
		<div
			ref={rootRef}
			className={`snake-root flex flex-col items-center gap-3 ${className}`}
		>
			<style jsx>{`
      ...
      `}</style>
			...
			>
				...
				<button
					type="button"
					className={`px-3 py-1 rounded border border-zinc-700 ${paused ? "bg-zinc-700" : "bg-zinc-800/80 hover:bg-zinc-700"}`}
				...
				<button
					type="button"
					className={`px-3 py-1 rounded border border-zinc-700 ${autoPlay ? "bg-emerald-700" : "bg-zinc-800/80 hover:bg-zinc-700"}`}
				...
				<button
					type="button"
					className={`px-3 py-1 rounded border border-zinc-700 ${showArrows ? "bg-fuchsia-700" : "bg-zinc-800/80 hover:bg-zinc-700"}`}
